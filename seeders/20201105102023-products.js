"use strict";

module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.bulkInsert(
      "Products",
      [
        {
          id: 1,
          name: "Drill",
          description: "Powerful drill, works on batteries.",
          pricePerDay: 2,
          numOfDaysAvailable: 5,
          userId: 1,
          categoryId: 4,
          picture: "public/pictures/photo-1504148455328-c376907d081c.jpeg",
          condition: "good",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 10)),
          updatedAt: new Date(),
        },
        {
          id: 2,
          name: "Tent",
          description: "2-person tent for camping.",
          pricePerDay: "10",
          numOfDaysAvailable: 7,
          userId: 4,
          categoryId: 5,
          picture: "public/pictures/photo-1585790339317-d3b8a421e6c9.jpeg",
          condition: "as good as new",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 4)),
          updatedAt: new Date(),
        },
        {
          id: 3,
          name: "Barbecue",
          description: "Charcoal BBQ. Cooking area: 40 cm.",
          pricePerDay: "5",
          numOfDaysAvailable: 4,
          userId: 5,
          categoryId: 2,
          picture: "public/pictures/photo-1463183427169-5df5a06d5447.jpeg",
          condition: "acceptable",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 15)),
          updatedAt: new Date(),
        },
        {
          id: 4,
          name: "Raclette",
          description: "For a delicious meal at home.",
          pricePerDay: "5",
          numOfDaysAvailable: 3,
          userId: 3,
          categoryId: 3,
          picture: "public/pictures/RaclettePrimeale.jpg",
          condition: "new",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 20)),
          updatedAt: new Date(),
        },
        {
          id: 5,
          name: "Electric guitar",
          description:
            "Awesome guitar to impress your guests with your skills.",
          pricePerDay: "10",
          numOfDaysAvailable: 3,
          userId: 3,
          categoryId: 5,
          picture: "public/pictures/jeremy-allouche-YStboKiFPVw-unsplash.jpg",
          condition: "acceptable",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 3)),
          updatedAt: new Date(),
        },
        {
          id: 6,
          name: "Floaties",
          description: "Fun at the beach or the pool.",
          pricePerDay: "3",
          numOfDaysAvailable: 7,
          userId: 2,
          categoryId: 5,
          picture: "public/pictures/toni-cuenca-55MySYrKf5w-unsplash.jpg",
          condition: "good",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 30)),
          updatedAt: new Date(),
        },

        {
          id: 7,
          name: "Mower",
          description: "Very powerful, make your garden looking good!",
          pricePerDay: "5",
          numOfDaysAvailable: 4,
          userId: 1,
          categoryId: 2,
          picture: "public/pictures/andres-siimon-zfwyrIA6bFw-unsplash.jpg",
          condition: "good",
          createdAt: new Date(),
          updatedAt: new Date(),
        },

        {
          id: 8,
          name: "Camera GoPro",
          description: "Action camera to document your best adventures!",
          pricePerDay: "3",
          numOfDaysAvailable: 7,
          userId: 4,
          categoryId: 5,
          picture: "public/pictures/photo-1484219372613-e9e83a68f732.jpeg",
          condition: "as good as new",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 8)),
          updatedAt: new Date(),
        },
        {
          id: 9,
          name: "Wedding dress",
          description: "Look beautiful on your special day. Size M",
          pricePerDay: "10",
          numOfDaysAvailable: 2,
          userId: 2,
          categoryId: 6,
          picture: "public/pictures/photo-1590272939560-b61891345b50.jpeg",
          condition: "good",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 22)),
          updatedAt: new Date(),
        },
        {
          id: 10,
          name: "Blender",
          description: "Makes delicious smoothies.",
          pricePerDay: "2",
          numOfDaysAvailable: 5,
          userId: 6,
          categoryId: 1,
          picture: "public/pictures/blender-10935_1280.jpg",
          condition: "good",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 7)),
          updatedAt: new Date(),
        },
        {
          id: 11,
          name: "Backpack",
          description: "45L",
          pricePerDay: "4",
          numOfDaysAvailable: 30,
          userId: 6,
          categoryId: 5,
          picture: "public/pictures/simon-migaj-0YLVlKSUEag-unsplash.jpg",
          condition: "good",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 28)),
          updatedAt: new Date(),
        },
        {
          id: 12,
          name: "Suitcase",
          description: "Lightweight suitcase.",
          pricePerDay: "5",
          numOfDaysAvailable: 30,
          userId: 2,
          categoryId: 6,
          picture: "public/pictures/travel-778338_1280.jpg",
          condition: "acceptable",
          createdAt: new Date(new Date().setDate(new Date().getDate() - 10)),
          updatedAt: new Date(),
        },
      ],
      {}
    );
  },

  down: async (queryInterface, Sequelize) => {
    await queryInterface.bulkDelete("Products", null, {});
  },
};
